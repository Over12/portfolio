  ...
  return (
    <>
      ...
      <AnimatePresence mode='wait'>
        {notification.show && (
          <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', stiffness: 400, damping: 30 }} className={`absolute text-sm sm:text-text right-0 bottom-16 sm:bottom-5 px-3 py-2 shadow-lg backdrop-brightness-200 border-l-4 ${notification.success ? 'border-green-500' : 'border-red-500'} cursor-none`}>
